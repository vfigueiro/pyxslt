...
def isWhitespaceNode(node):
    ...
    return node.nodeType == xml.dom.Node.TEXT_NODE and bool(
        re.match(r'[ \t\r\n]*$', node.data)
    )
...
def isNameTestInList(test, l):
    ...
    return test in l or (test[0], None) in l or (None, None) in l
...
def stripSpace(
    ...
    if node.nodeType == xml.dom.Node.TEXT_NODE:
        ...
        childNodes = list(
            node.childNodes
        )  # save ChildNodes. deleting a child will modify node.childNodes
...
def pushResult(value, result):
    ...
    if isinstance(result, StringIO) or result.nodeType in [
    ...
    if isinstance(result, xml.dom.Node):
        if result.nodeType in [
            ...
            if (
                isinstance(value, xml.dom.Node)
                and value.nodeType == xml.dom.Node.ATTRIBUTE_NODE
            ):
                if (
                    result.nodeType != xml.dom.Node.ELEMENT_NODE
                    or result.childNodes.length > 0
                ):
            ...
            if (
                isinstance(value, xml.dom.Node)
                and value.nodeType == xml.dom.Node.DOCUMENT_NODE
            ):
...
def combineOutput(output, newvalues):
    ...
    if newvalues.has_key('cdata-section-elements') and output.has_key(
...
def computeStringDefaultPriority(expr):
    ...
    if re.match(
        AXIS_RE + r'(?:' + QNAME_RE + r'|processing-instruction([^"]*)$', self.match_s
    ):
        self.priority = 0
...
def computeDefaultPriority(pattern):
    ...
    if (
        type(pattern) is X.AbsolutePathExpr
        and type(pattern.path) is X.PathExpr
        and len(pattern.path.steps) == 2
        and type(pattern.path.steps[0]) is X.AxisStep
        and type(pattern.path.steps[0].test) is X.AnyKindTest
        and pattern.path.steps[0].axis.__name__ == 'descendant-or-self'
        and type(pattern.path.steps[1]) is X.AxisStep
    ):
        ...
        if type(nodeTest) is X.NameTest:
            if nodeTest.localName != '*':  # ChildOrAttributeAxisSpecifier QName
                return 0
            if (
                nodeTest.prefix != '*' and nodeTest.localName == '*'
            ):  # ChildOrAttributeAxisSpecifier NCName:*
                return -0.25
            return -0.5  # *
