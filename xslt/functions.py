...
def f_string(node, pos, size, context, v):
    if (
        isinstance(v, xml.dom.Node)
        and v.nodeType == xml.dom.Node.DOCUMENT_FRAGMENT_NODE
    ):
...
def f_document(node, pos, size, context, obj, nodeset=None):
    if xpath.tools.nodesetp(obj):
        ...
        for x in obj:
            r += f_document(
                node, pos, size, context, xpath.tools.string(x, context), nodeset or [x]
            )
        return r
    ...
    if nodeset is None:
        ...
        if xpath.tools.nodesetp(nodeset):
            baseUri = (
                nodeset[0].ownerDocument.baseUri
                if nodeset[0].nodeType != xml.dom.Node.DOCUMENT_NODE
                else nodeset[0].baseUri
            )
        elif (
            isinstance(nodeset, xml.dom.Node)
            and nodeset.nodeType == xml.dom.Node.DOCUMENT_FRAGMENT
        ):
