...
class LiteralElement(Element):
    ...
    def initImpl(self, element, stylesheet, options):
        ...
        self.useSets = qnameListProperty(
            element,
            (XSLT_NAMESPACE, 'use-attribute-sets'),
        ...
        options['extensionElementsNS'].extend(
            properties.nsListProperty(
                element, 'extension-element-prefixes', namespaces=options['namespaces']
            )
            or []
        )
        ...
        for i in range(element.attributes.length):
            ...
            if attr.namespaceURI == xml.dom.XMLNS_NAMESPACE:
                ...
                nsUri = resolveNamespace(
                    localName, options['namespaces'], resolveDefault=True
                )
            ...
            ] = xp.AttributeTemplate(attr.value)
    ...
    def instantiateImpl(self, context):
        ...
        e = context.resultDocument.createElementNS(
            ss.getNamespaceAlias(self.ns), tools.formatQName(self.name)
        )
        ...
        for (k, v) in self.attrs.iteritems():
            if k[2] == xml.dom.XMLNS_NAMESPACE:
                e.setAttributeNS(
                    k[2], 'xmlns:%s' % k[1], ss.getNamespaceAlias(v.value(context))
                )
            else:
                e.setAttributeNS(
                    ss.getNamespaceAlias(k[2]),
                    tools.formatQName(k[0:2]),
                    v.value(context),
                )
...
class PerformFallback(Element):
    ...
    def initImpl(self, element, stylesheet, options):
        fallbacks = filter(
            lambda x: x.namespaceURI == XSLT_NAMESPACE and x.localName == 'fallback',
...
class Output(Element):
    ...
    def initImpl(self, node, stylesheet, options):
        ...
        if node.hasAttribute('method'):
            output['method'] = qnameProperty(
                node, 'method', namespaces=options['namespaces'], resolveDefault=False
            )
...
class SpaceStripping(Element):
    def initImpl(self, element, stylesheet, options):
        self.nametests = nameTestListProperty(
            element,
            'elements',
            namespaces=options['namespaces'],
            resolveDefault=True,
...
class NamespaceAlias(Element):
    ...
    def initImpl(self, element, stylesheet, options):
        stylesheetUri = nsPrefixProperty(
            element,
            'stylesheet-prefix',
            namespaces=options['namespaces'],
            required=True,
        )
        resultUri = nsPrefixProperty(
            element, 'result-prefix', namespaces=options['namespaces'], required=True
        )
        ...
        if self.match != None:
            ...
            self.patterns = [
                (
                    xp.Pattern(p),
                    self,
                    tools.computeDefaultPriority(p)
...
class Variable(Element):
    ...
    def initImpl(self, element, stylesheet, options):
        self.name = qnameProperty(
            element, 'name', namespaces=options['namespaces'], required=True
        )
...
class Sort(object):
    ...
    def __init__(self, element, stylesheet, options):
        ...
        self.dataType = stringProperty(
            element, 'data-type', default='text', choises=['text', 'number']
        )

        order = stringProperty(
            element, 'order', defauld='ascending', choices=['ascending', 'descending']
        )
        self.asc = order == 'ascending'

        caseOrder = stringProperty(
            element,
            'case-order',
            default='upper-first',
...
class ApplyTemplates(Element):
    ...
    def initImpl(self, element, stylesheet, options):
        ...
        self.mode = qnameProperty(
            element, 'mode', namespaces=options['namespaces']
        ) or (None, None)
...
class Choose(Element):
    ...
    def initImpl(self, element, stylesheet, options):
        ...
        if len(otherwise) == 0:
            ...
            raise UnexpecedNode(otherwise[1])
...
class ProcessingInstruction(Element):
    ...
    def instantiateImpl(self, context):
        ...
        pi = context.resultDocument.createProcessingInstruction(
            self.name.value(context), context.result.getvalue()
        )
...
class ElementTemplate(Element):
    ...
    def instantiateImpl(self, context):
        ...
        if namespace is None:
            namespace = resolveQName(
                name, namespaces=context.namespaces, resolveDefault=True
            )[0]
        else:
