...
def xsltHeader(node, allowedNames=[]):
    ...
    for i in range(node.childNodes.length):
        ...
        if tools.isWhitespaceNode(child):
            ...
            child.nodeType == xml.dom.Node.ELEMENT_NODE
            and child.namespaceURI == XSLT_NAMESPACE
            and child.localName in allowedNames
        ):
...
def fixNamespaces(node):
    ...
    if node.nodeType == xml.dom.Node.ELEMENT_NODE:
        ...
        if node.namespaceURI not in ns.values():
            node.setAttributeNS(
                xml.dom.XMLNS_NAMESPACE, 'xmlns:' + node.prefix, node.namespaceURI
            )
        for i in range(node.attributes.length):
            attr = node.attributes.item(i)
            if (
                attr.namespaceURI not in ns.values()
                and attr.namespaceURI != xml.dom.XMLNS_NAMESPACE
            ):
                node.setAttributeNS(
                    xml.dom.XMLNS_NAMESPACE, 'xmlns:' + attr.prefix, attr.namespaceURI
                )
    else:
                ...
                if templateNode.namespaceURI == XSLT_NAMESPACE:
                    if templateNode.localName in TemplateContent.allowedElements:
                        ...
                        if options['forwardsCompatible']:
                            e = self._ef.new(
                                '_perform_fallback', templateNode, stylesheet, options
                            )
                        else:
                    ...
                    if templateNode.namespaceURI in options['extensionElementsNS']:
                        try:
                            e = self._ef.new(
                                (templateNode.namespaceURI, templateNode.localName),
                                templateNode,
                                stylesheet,
                                options,
                            )
                        ...
                        e = self._ef.new(
                            '_literal_element', templateNode, stylesheet, options
                        )
