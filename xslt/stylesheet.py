...
class Stylesheet(object):
    def __init__(self, uriOrDoc=None, dp=core.DocumentProvider()):
        ...
        for imp in self.imports:  # descending import precedence
            ...
            mergeNameTestLists(
                self.stripSpace, imp.stripSpace, self.stripSpace + self.preserveSpace
            )
            mergeNameTestLists(
                self.preserveSpace,
                imp.preserveSpace,
    ...
    def _parseStylesheetContent(self, doc, baseUri):
        ...
        if sheet.namespaceURI != core.XSLT_NAMESPACE or not (
            sheet.localName == 'stylesheet' or sheet.localName == 'transform'
        ):
        ...
        options['extensionElementsNS'] = (
            properties.nsListProperty(
                sheet, 'extension-element-prefixes', namespaces=options['namespaces']
            )
            or []
        )
        options['excludeResultNS'] = (
            properties.nsListProperty(
                sheet, 'exclude-result-prefixes', namespaces=options['namespaces']
            )
            or []
        )
        ...
        for i in range(sheet.childNodes.length):
            ...
            if (
                node.namespaceURI != core.XSLT_NAMESPACE
            ):  # Ignore top-level elements with an unknown non-null namespace [XSLT 2.2]
            ...
            if node.localName == 'include':
                ...
                self._parseStylesheetContent(
                    self.dp.document(href, baseUri), self.dp.absUri(href, baseUri)
                )

            if node.localName == 'variable' or node.localName == 'param':
                varclass = (
                    elements.Param if node.localName == 'param' else elements.Variable
                )
    ...
    def applyTemplatesImpl(self, context, mode):
        ...
        if (
            context.node.nodeType == Node.ELEMENT_NODE
            or context.node.nodeType == Node.DOCUMENT_NODE
        ):
            ...
            context.node.nodeType == Node.TEXT_NODE
            or context.node.nodeType == Node.CDATA_SECTION_NODE
            or node.nodeType == Node.ATTRIBUTE_NODE
        ):
