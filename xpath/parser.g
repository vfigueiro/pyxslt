...
parser XPath:
	...
	# HAAAAACKS. Regexps for just tokenizing!!! (- Cyril)
    token FORWARD_AXIS_NAME:
        r'child(?=\s*::)|descendant-or-self(?=\s*::)|attribute(?=\s*::)|self(?=\s*::)|descendant(?=\s*::)|following-sibling(?=\s*::)|following(?=\s*::)|namespace(?=\s*::)'
    token REVERSE_AXIS_NAME:
        r'parent(?!\s*\()|preceding-sibling(?!\s*\()|preceding(?!\s*\()|ancestor-or-self(?!\s*\()|ancestor(?!\s*\()'
    ...
    token LITERALTEXT:   r'[^\{]*' # for attribute value template
    ...
    # XSLT Patterns [XSLT 5.2]

    rule Pattern:
        LocationPathPattern             {{ Expr = LocationPathPattern }}
        (
    ...
    rule LocationPathPattern:
          r'\/'                         {{ path = None }}
          [
              RelativePathPattern       {{ path = RelativePathPattern }}
          ]                             {{ return X.AbsolutePathExpr(path) }}
        | [ r'\/\/' ]
          RelativePathPattern           {{ step = X.AxisStep('descendant-or-self') }}
                                        {{ RelativePathPattern.steps.insert(0, step) }}
                                        {{ return X.AbsolutePathExpr(RelativePathPattern) }}
        | IdKeyPattern                  {{ expr = [ IdKeyPattern ] }}
          [
            (
                r'\/'                   {{ step = None }}
              | r'\/\/'                 {{ step = X.AxisStep('descendant-or-self') }}
            )
            RelativePathPattern         {{ if step: RelativePathPattern.steps.append(step) }}
    ...
    rule RelativePathPattern:
        ...
        (
          (
              r'\/'
            | r'\/\/'                   {{ steps.append(X.AxisStep('descendant-or-self')) }}
          )
    ...
    rule StepPattern:
        (
          ...
          | ChildOrAttributeAxisStep    {{ step = ChildOrAttributeAxisStep }}
        )                               {{ expr = X.AxisStep(*step) }}
        [
...
# Attribute value templates
    ...
    rule AttributeValueTemplate:
      LITERALTEXT                      {{ args = [ X.LiteralExpr(LITERALTEXT) ] }}
      (
        [
          r'\{'
          Expr                          {{ args.append(Expr) }}
        ...
        LITERALTEXT                    {{ args.append(X.LiteralExpr(LITERALTEXT)) }}
      )*
      END                               {{ return X.Function('concat', args) }}
