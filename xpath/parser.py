...
class XPathScanner(Scanner):
    patterns = [
        ...
        ("'child'", re.compile('child')),
        ("r'key'", re.compile('key')),
        ("r'id'", re.compile('id')),
        ("r'\\:'", re.compile('\\:')),
        ...
        (
            'FORWARD_AXIS_NAME',
            re.compile(
                'child(?=\\s*::)|descendant-or-self(?=\\s*::)|attribute(?=\\s*::)|self(?=\\s*::)|descendant(?=\\s*::)|following-sibling(?=\\s*::)|following(?=\\s*::)|namespace(?=\\s*::)'
            ),
        ),
        (
            'REVERSE_AXIS_NAME',
            re.compile(
                'parent(?!\\s*\\()|preceding-sibling(?!\\s*\\()|preceding(?!\\s*\\()|ancestor-or-self(?!\\s*\\()|ancestor(?!\\s*\\()'
            ),
        ),
...
class XPath(Parser):
    ...
    def AndExpr(self):
        ...
        Expr = EqualityExpr
        while (
            self._peek("r'and'", "r'or'", 'END', "r'\\]'", "r'\\)'", "r'\\,'", "r'\\}'")
            == "r'and'"
        ):
            self._scan("r'and'")
            EqualityExpr = self.EqualityExpr()
    ...
    def KindTest(self):
        _token_ = self._peek(
            "r'processing-instruction'", "r'comment'", "r'text'", "r'node'"
        )
    ...
    def StepPattern(self):
        ...
        expr = X.AxisStep(*step)
        if self._peek("r'\\['", "r'\\/'", "r'\\/\\/'", "'\\|'", 'END') == "r'\\['":
            PredicateList = self.PredicateList()
            expr = X.PredicateList(expr, PredicateList, step[0])
        return expr

    def ChildOrAttributeAxisStep(self):
        _token_ = self._peek("'child'", "'attribute'")
        if _token_ == "'child'":
            self._scan("'child'")
            axis = 'child'
        else:  # == "'attribute'"
