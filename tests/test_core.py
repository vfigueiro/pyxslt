...
class TestAssertEmptyNode(unittest.TestCase):
    ...
    def testSimple(self):
        self.assertEqual(
            None, assertEmptyNode(self.root.childNodes[0]), "empty element"
        )
        self.assertEqual(None, assertEmptyNode(self.root.childNodes[1]))
        self.assertRaises(
            xslt.exceptions.UnexpectedNode, assertEmptyNode, self.root.childNodes[2]
        )
...
class TestXsltChildren(unittest.TestCase):
    def setUp(self):
        self.doc = xml.dom.minidom.parseString(
            '<root xmlns:s="http://www.w3.org/1999/XSL/Transform">'
            + '<test> <s:a/>\r\n<s:b/>\t</test>'
            + '<test> <s:a/><s:b/><c/></test>'
    ...
    def testFailElement(self):
        ...
        self.assertRaises(
            xslt.exceptions.UnexpectedNode, xsltChildren, test, ['a', 'b']
        )
...
class TestXsltHeader(unittest.TestCase):
    ...
    def testSkipWhitespace(self):
        ...
        self.assertTrue(r[1] is test.childNodes[3])
        self.assertEquals(
            t, [test.childNodes[0], test.childNodes[2], test.childNodes[4]]
        )
...
class TestTemplateContent(unittest.TestCase):
    ...
    def testAddNodes(self):
        ...
        self.assertEquals(
            tc.children,
            ['<xsl:copy>', '_literal_text', '<xsl:value-of>', '_literal_element'],
        )

        tc = self.createMockedTC(1, options)
        self.assertRaises(
            xslt.exceptions.NotImplemented,
            tc.addNodes,
            root.childNodes[4:5],
            1,
            options,
        ...
        tc = self.createMockedTC(1, options)
        self.assertRaises(
            xslt.exceptions.NotImplemented, tc.addNodes, root.childNodes[5:], 1, options
        )
