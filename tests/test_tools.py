...
class TestStripSpace(unittest.TestCase):
    ...
    def testXmlSpace(self):
        ...
        self.assertEquals(
            self.r.getvalue(), '<b xml:space="preserve"> <b xml:space="default"/> </b>'
        )
    ...
    def testNamespaces(self):
        ...
        node.writexml(self.r)
        self.assertEquals(
            self.r.getvalue(), '<b><u xmlns="uri:preserve"> <b/> </u></b>'
        )
...
class TestCombineOutput(unittest.TestCase):
    def testSimple(self):
        ...
        combineOutput(
            a,
            {
                'cdata-section-elements': set([('uri:test', 'b')]),
                'method': 'xml',
                'version': '1.0',
            },
        )
        self.assertEquals(
            a['cdata-section-elements'], set([(None, 'a'), ('uri:test', 'b')])
        )
...
class TestGetNamespaceBindings(unittest.TestCase):
    def setUp(self):
        self.doc = xml.dom.minidom.parseString(
            '<a>' + '<test xmlns:z="b"/>' + '<test/>' + '<test xmlns="a"/>' + '</a>'
        )
        self.root = self.doc.documentElement
...
class TestMergeNameTestLists(unittest.TestCase):
    ...
    def testOverriding(self):
        ...
        self.assertEquals(
            [('b', 'c')], mergeNameTestLists([], [('b', 'c')], [('a', None)])
        )
        self.assertEquals([], mergeNameTestLists([], [('a', None)], [('a', None)]))
        self.assertEquals(
            [(None, None)], mergeNameTestLists([], [(None, None)], [('a', None)])
        )
